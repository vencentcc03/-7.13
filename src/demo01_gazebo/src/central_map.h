...
public:
    ...
    std::vector<std::vector<double>> start_coordinates_2; // dilatioon之后的起始坐标
    std::vector<std::vector<double>> target_coordinates;
    std::vector<std::vector<double>> target_coordinates_2; // dilation之后的目标坐标
    ...
    std::vector<int> centerPoint_target;
    ...
    void find_centerpoint_target();
    std::vector<double> coordinate_dilation(double coox, double cooy);
    void set_boundary();
    void dilation();
